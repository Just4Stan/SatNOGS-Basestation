...
bool cc1200_hal_init(cc1200_hal_t* hal)
{
    ...
    // >=100 ns inter-byte delay for config register burst writes (CC120x requirement)
    // Add +2 cycle margin to guarantee we exceed 100 ns
    const uint32_t sys_hz = clock_get_hz(clk_sys);
    uint32_t cycles = sys_hz / 10000000u;  // cycles for ~100 ns
    if (cycles < 2) cycles = 2;
    hal->cfg_burst_interbyte_delay_cycles = cycles + 2;
