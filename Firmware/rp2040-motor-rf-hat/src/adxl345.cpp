...
ADXL345::ADXL345(uint cs_pin, uint sck_pin, uint mosi_pin, uint miso_pin)
    : cs_(cs_pin), sck_(sck_pin), mosi_(mosi_pin), miso_(miso_pin),
      ok_(false), spi_initialized_(false) {}

void ADXL345::acquire_spi() {
  // Switch SPI0 from VHF CC1200 mode (mode 0, 5 MHz) to ADXL345 mode (mode 3, 1 MHz).
  spi_deinit(spi0);
  spi_init(spi0, 1000000);
  spi_set_format(spi0, 8, SPI_CPOL_1, SPI_CPHA_1, SPI_MSB_FIRST);

  // SPI pins are already configured as SPI function from VHF init.
  // Just need to make sure they stay as SPI.
  gpio_set_function(sck_, GPIO_FUNC_SPI);
  gpio_set_function(mosi_, GPIO_FUNC_SPI);
  gpio_set_function(miso_, GPIO_FUNC_SPI);
}

void ADXL345::release_spi() {
  // Switch SPI0 back to VHF CC1200 mode (mode 0, 5 MHz).
  spi_deinit(spi0);
...
bool ADXL345::init() {
  // For first init, SPI0 may not be set up yet. Configure it for mode 3.
  spi_init(spi0, 1000000);
  spi_set_format(spi0, 8, SPI_CPOL_1, SPI_CPHA_1, SPI_MSB_FIRST);
  ...
  if (id != 0xE5) {
    ok_ = false;
    // Release SPI back to mode 0 for VHF CC1200
    release_spi();
    return false;
  }
  ...
  // Leave SPI in mode 3 during init — caller will switch to mode 0 for VHF
  // after homing is complete. Don't release_spi() here because the homing
  // loop needs to keep reading the IMU.
  return true;
}
