...
\begin{figure}[H]
    ...
    \caption{Slew speed vs. duty cycle, single axis, 24~V, unloaded.}
...
\begin{figure}[H]
    ...
    \caption{PID error vs. time for 75\,\%, 85\,\%, and 95\,\% duty caps on the same 90\degree\ step.}
...
\begin{figure}[H]
    ...
    \caption{PID performance metrics per test run.}
...
\begin{figure}[H]
    ...
    \caption{Tracking error vs. time across a full satellite pass.}
...
\begin{figure}[H]
    ...
    \caption{Tracking error vs. peak elevation, one point per pass.}
...
\begin{figure}[H]
    ...
    \caption{Polar sky plot of all tracked passes from the April 9 session.}
    \label{fig:allpasses}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{RSSI distribution indoors (lab) vs. outdoors (open field).}
    \label{fig:rssi-indoor-outdoor}
\end{figure}

\textbf{Doppler correction.} Full CC1200 \code{FREQ} word rewritten every 500~ms from the TLE-derived radial velocity; the \code{FREQOFF} register is read back only as a residual-error estimate. The observed $\pm$9~kHz sweep at 432~MHz is consistent with the $\pm$11~kHz maximum at 433~MHz predicted in \S\ref{sec:doppler} (the commanded offset is clipped at the mid-pass peak that the satellite actually reaches).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/doppler_real_pass.png}
    \caption{Doppler correction during a real pass: commanded FREQOFF vs. time.}
...
\begin{figure}[H]
    ...
    \caption{Mean peak RSSI binned by peak elevation across 62 tracked passes.}
...
\begin{figure}[H]
    ...
    \caption{Negative control: same binning applied to the April 1 session, pre-calibration.}
...
\begin{figure}[H]
    ...
    \caption{Cross-capture byte-match histogram for SNUGLITE-II across six pairwise comparisons.}
...
\begin{figure}[H]
    ...
    \caption{RSSI vs. slant range for the six highest-dynamic-range passes.}
