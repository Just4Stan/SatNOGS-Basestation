...
\begin{figure}[H]
    ...
    \caption{PID step response overlay for a 90\degree\ commanded step.}
...
\begin{figure}[H]
    ...
    \caption{PID settling detail, zoomed to $\pm$1\degree\ around the setpoint.}
...
\begin{enumerate}
    ...
    \item Apply Doppler correction at 2~Hz by writing the full CC1200 \code{FREQ} word; the \code{FREQOFF} register is read back only as a residual-error estimate.
    \item Drain RX packets and log telemetry (position, RSSI, freq, Doppler, range) to CSV at 1~Hz.
    \item Write \file{\textasciitilde/.station\_status.json} at 1~Hz for the dashboard.
\end{enumerate}
...
The setup wizard has five steps: (1) GPS fix $\to$ Pi (typically supplied by a phone browser, which has a GPS sensor), (2) point the station north and confirm with the ``Use Phone Compass'' button (which calls \code{onboardCalibrate()} over the EasyComm socket), (3) attach the antenna (the station is level from IMU homing), (4) select receiver type, (5) ready. Tracking begins automatically after step 5.
...
\begin{figure}[H]
    ...
    \caption{\code{station.py} data flow from TLE propagation to rotator, CC1200, and dashboard.}
...
\begin{figure}[H]
    ...
    \caption{Field deployment workflow from tripod placement to auto-tracking.}
...
\begin{figure}[H]
    ...
    \caption{Station deployed in the field, April 9 session.}
