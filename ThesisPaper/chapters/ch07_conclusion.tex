...
\begin{enumerate}
    ...
    \end{itemize}
    Combined impact: system NF $\sim$7~dB $\to$ $<$1~dB from the LNA, and the LNA's 20~dB gain lifts the received signal from $-$125.7~dBm to $-$105.7~dBm at the CC1200 input. With the measured field noise floor at $-$96~dBm this still leaves approximately $-$10~dB margin, so board-level EMI mitigation (v2 PSU layout, enclosure shielding, or both) is required alongside the LNA to close the link. v2 PCB change: add a bias-T network on the UHF SMA port; the LNA itself stays off-board.
