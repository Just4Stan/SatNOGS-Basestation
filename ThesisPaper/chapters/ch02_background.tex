...
EasyComm is an ASCII protocol developed by Chris Jackson (G7UPN) for antenna tracking. The SatNOGS variant uses hamlib model 204 (EasyComm III):
...
A single station contacts a LEO satellite for at most $\sim$10\% of each orbit. For mission-critical telemetry this is not enough. SatNOGS solves this by distributing stations worldwide so nearly every orbit has coverage. Previous attempts --- the Japanese Ground Station Network (University of Tokyo) and ESA's GENSO (Global Educational Network for Satellite Operations, started 2006, cancelled by 2014) --- failed because they required homogeneous hardware and centralized coordination. SatNOGS accepts any hardware that speaks EasyComm and uses an SDR receiver.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{llcl>{\raggedright\arraybackslash}X}
        ...
        \midrule
        Yaesu G-5500 & Commercial & 760--900 & $\sim$1\degree\ backlash & Industry standard, potentiometer feedback \\
        SPID RAS & Commercial & 1200--1300 & $\sim$1\degree & Also sold as Alfa-SPID \\
        SatNOGS v3 & Open-source & 300--500 & $\sim$1\degree & 3D-printed, NEMA17 steppers, belt drive \\
        SuperAntennaz & Open-source & $\sim$600 & $\sim$1\degree & Heavy-load stepper design \\
        \textbf{This work} & \textbf{Open-source} & \textbf{$<$50} & \textbf{$<$0.5\degree\ EL} & \textbf{ASA 3D-printed, geared DC motors, 0--3\degree\ AZ} \\
        \bottomrule
    \end{tabularx}
...
The official SatNOGS rotator uses two NEMA17 bipolar steppers with GT2 belt reduction, controlled by an Arduino Pro Mini with A4988 or DRV8825 drivers. BOM: EUR~300--500. Known limitations:

\begin{itemize}
    \item Steppers draw 0.8--1.5~A per phase holding current to resist back-driving---problematic for battery operation.
...
Most university ground stations (UPSat/University of Patras, TU Delft/Delfi-C3, AraMiS/Politecnico di Torino) buy commercial rotators and focus on RF or software. Few attempt custom mechanical design. A notable open-source exception is SuperAntennaz (Hadji), designed for heavy antenna loads ($>$10~kg) using stepper motors.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X}
        ...
        Pi integration & Theoretical description & Python stack ($\sim$7200 LOC) + web dashboard \\
        ...
        Tracking & Not demonstrated & Live tracking validated ($<$0.5\degree\ EL, 0--3\degree\ AZ) \\
        \bottomrule
    \end{tabularx}
