...
University CubeSat programmes are proliferating: a 3U spacecraft can carry a meaningful scientific payload, be assembled by a small student team, and reach orbit on a rideshare for a fraction of the cost of a dedicated launch. KU Leuven's AetherSpace mission is one such programme, targeting a heat-shield sample-return demonstration from LEO around 2030. Once in orbit, however, a CubeSat is only as useful as the ground infrastructure that receives its telemetry. Commercial antenna tracking systems suitable for a 433\,MHz LEO link cost \euro{}760--1300; even the community-built SatNOGS v3 reference design requires \euro{}300--500 in parts. For a student team without a permanent rooftop installation, this cost and the associated setup complexity make routine contact difficult.

This thesis addresses that gap by designing, building, and field-validating a portable, SatNOGS-compatible ground station that can be assembled for approximately \euro{}100 in station-core components, deploys from a standard camera tripod in about four minutes, and integrates with the SatNOGS rotator ecosystem through the standard EasyComm\,III interface. The target application is the AetherSpace CubeSat, but the rotator and software stack are general-purpose: any satellite reachable from the SatNOGS DB can be tracked, and any operator with a hamlib-compatible rotator can reuse the software.
