...
\begin{figure}[H]
    ...
    \caption{The assembled and field-deployed ground station tracking a pass. Tripod-mounted rotator with Siretta Oscar 44 Yagi on the EL axis.}
...
\begin{enumerate}
    ...
    \item \textbf{Form-factor fit.} A Pi 3A+ ships with 512~MB of LPDDR2 SDRAM~\cite{rpi3aplus}; with the default 64~MB GPU split, approximately 416~MB is available to userspace. The SatNOGS Docker stack targets $\geq$1~GB of RAM~\cite{satnogs-main}. The Python stack described in \S\ref{sec:pi-stack} ($\sim$7400 LOC across the Pi packages listed in Appendix~\ref{app:bom}) runs on the 3A+ within a 200--300~MB resident-set footprint measured with \code{top}, leaving margin for the HTTPS dashboard and systemd services.
\end{enumerate}
