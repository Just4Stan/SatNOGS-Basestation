...
Four constraints shape the architecture. Portability is enforced by a 1/4"-20 UNC tripod thread on the base, with the full setup measured at approximately 4 minutes from unpacking to first tracked pass across three field sessions (\S\ref{sec:field-tests}). The zero-SSH workflow routes every field operation, GPS entry, north alignment, pass selection, park, and shutdown, through the HTTPS dashboard, which targets a laptop browser but also works on a phone for quick tasks such as supplying a GPS fix. The cost target is the v2 MOTOR\_RF\_HAT BOM derived in \S\ref{sec:cost} and listed in Appendix~\ref{app:bom}: $\sim$€40 for the combined controller/RF PCB, $\sim$€75 for the rotator hardware, $\sim$€100 for the station core (excluding antenna, coax, power supply, tripod).
...
The elevation axis is a PVC pipe passing through both side plates. The elevation gear and any other parts mounted to the pipe are fixed in place with three radial set screws each, which can be slid along the pipe during assembly to set mesh with the motor pinion before being tightened.

All fasteners are M4$\times$10~mm countersunk screws (used both for structural joints and as set screws on the elevation-pipe clamps) and M3$\times$8~mm for motor mounts. The assembly uses no adhesives, so the station disassembles for transport or repair without destructive separation.
...
Component prices from the LCSC BOM tool (April 2026), bare PCB from JLCPCB (5-piece order). Full BOM in Appendix~\ref{app:bom}.
...
\subsection{v2 (MOTOR\_RF\_HAT, initial design-complete)}
